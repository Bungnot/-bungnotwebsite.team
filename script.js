...
/* ================================
   Cloud LiveSummary Sync (Viewer)
   - รวมยอดจาก input ทั้งหมด -> Firebase: liveSummary
   - ส่งเวลาล่าสุด -> Firebase: liveSummaryUpdatedAt
   วางไว้ "ท้ายสุด" ของ script.js
================================== */
...
(function () {
  ...
  function computeLiveSummaryFromInputs() {
    const summary = {};
    ...
    document.querySelectorAll(".table-container tbody tr").forEach(tr => {
      ...
      const chaser = (inputs[0].value || "").trim();
      const priceRaw = (inputs[1].value || "").replace(/[Oo]/g, "0");
      const holder = (inputs[2].value || "").trim();

      const nums = priceRaw.match(/\d+/g) || [];
      let rowTotal = 0;
      nums.forEach(n => { if (n.length >= 3) rowTotal += parseInt(n, 10); });

      if (rowTotal > 0) {
        if (chaser) summary[chaser] = (summary[chaser] || 0) + rowTotal;
        if (holder && holder !== chaser) summary[holder] = (summary[holder] || 0) + rowTotal;
      }
    });
  ...
  function pushLiveSummaryNow() {
    try {
      const summary = computeLiveSummaryFromInputs();
      db.ref("liveSummary").set(summary);
      db.ref("liveSummaryUpdatedAt").set(Date.now());
    } catch (e) {
      console.warn("LiveSummary push failed:", e);
    }
  }

  function schedulePush() {
    clearTimeout(__timer);
    __timer = setTimeout(pushLiveSummaryNow, 600);
  }

  function patchWhenDbReady() {
    if (__patched) return true;

    // db มาจาก index.html (firebase.initializeApp แล้ว const db = firebase.database();) :contentReference[oaicite:5]{index=5}
    if (typeof db === "undefined" || !db || typeof db.ref !== "function") return false;

    // hook เข้า saveData() (คุณเรียก saveData ตอนพิมพ์ในช่องตาราง) :contentReference[oaicite:6]{index=6}
    if (typeof saveData === "function") {
      const __origSaveData = saveData;
      saveData = function () {
        __origSaveData();
        schedulePush();
      };
    } else {
      // fallback เผื่อกรณีหา saveData ไม่เจอ
      setInterval(pushLiveSummaryNow, 2000);
    }
    ...
    __patched = true;
    console.log("[LiveSummary] patched OK");
    return true;
  }

  // รอให้ db ถูกสร้าง (เพราะ db ถูกประกาศหลังโหลด script.js ใน index.html) :contentReference[oaicite:7]{index=7}
  function bootWaitForDb(maxMs = 8000) {
    const start = Date.now();
    const tick = () => {
      if (patchWhenDbReady()) return;
      if (Date.now() - start > maxMs) {
        console.warn("[LiveSummary] db not ready (timeout). Check firebase init order.");
        return;
      }
      setTimeout(tick, 100);
    };
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => bootWaitForDb());
  } else {
    bootWaitForDb();
  }
})();
