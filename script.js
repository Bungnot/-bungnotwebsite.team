...
/* =========================================================
   ✅ FIX: MULTI-ADMIN SAME VIEW (Snapshot Sync via Firebase)
   วางท้ายสุดของ script.js (ล่างสุดจริงๆ)
========================================================= */
(function () {
  if (window.__rocketSnapshotSyncInstalled) return;
  window.__rocketSnapshotSyncInstalled = true;

  const SNAP_PATH = "adminRocketShared/v1/savedTablesSnapshot"; // ที่เก็บ snapshot ตาราง
  const REV_KEY   = "savedTables_rev_snapshot";                  // rev ใน localStorage กันวนลูป

  // client id (กันชนกัน/ดูว่าอัปเดตมาจากใคร)
  const CID_KEY = "rocket_client_id";
  ...
  // ดึง db แบบปลอดภัย (เพราะ db ถูกประกาศใน index.html หลังโหลด script.js)
  function getDbSafe() {
  ...
  function publishSnapshotNow() {
    ...
    let data = [];
    try {
      data = JSON.parse(localStorage.getItem("savedTables") || "[]");
    } catch { data = []; }

    const rev = Date.now();
    localStorage.setItem(REV_KEY, String(rev));

    _db.ref(SNAP_PATH).set({
      rev,
      updatedAt: Date.now(),
      updatedBy: clientId,
      data
  ...
  function publishSnapshotDebounced() {
    if (publishTimer) clearTimeout(publishTimer);
    publishTimer = setTimeout(publishSnapshotNow, 250);
  }

  // ✅ PATCH saveData: หลังบันทึก localStorage แล้ว → publish snapshot ไป Firebase
  if (typeof window.saveData === "function" && !window.saveData.__snapshotWrapped) {
    const _saveData = window.saveData;
    function wrappedSaveData() {
      if (applyingRemote) return;  // กัน loop ตอนดึงของคนอื่นมา
      _saveData.apply(this, arguments);
      publishSnapshotDebounced();
    }
    wrappedSaveData.__snapshotWrapped = true;
    window.saveData = wrappedSaveData;
  }

  function refreshUIAfterRemote() {
    try { if (typeof loadData === "function") loadData(); } catch {}
  ...
  function applyRemoteSnapshot(payload) {
    ...
    const localRev  = toInt(localStorage.getItem(REV_KEY) || "0");

    if (remoteRev <= localRev) return;

    applyingRemote = true;
    try {
      localStorage.setItem("savedTables", JSON.stringify(payload.data));
      localStorage.setItem(REV_KEY, String(remoteRev));
      refreshUIAfterRemote();
    } finally {
  ...
  function startSnapshotSync() {
    const _db = getDbSafe();
    if (!_db) {
      console.warn("Snapshot Sync: db ยังไม่พร้อม (Firebase ยังไม่ init)");
      return;
    }

    // 1) subscribe
    _db.ref(SNAP_PATH).on("value", (snap) => {
      applyRemoteSnapshot(snap.val());
    });

    // 2) init: ถ้าใน Firebase ยังไม่มี snapshot → seed จากเครื่องนี้ 1 ครั้ง
    _db.ref(SNAP_PATH).once("value").then((snap) => {
      if (!snap.exists()) {
        // ยังไม่มีข้อมูลกลาง → ปล่อยให้เครื่องแรกเป็นตัวตั้งต้น
        publishSnapshotNow();
      } else {
        applyRemoteSnapshot(snap.val());
      }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // กันกรณี refresh แล้ว rev หาย
    if (!localStorage.getItem(REV_KEY)) localStorage.setItem(REV_KEY, "0");
    setTimeout(startSnapshotSync, 0);
  });
})();
